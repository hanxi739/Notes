...
存储无序的、不可重复的数据。Set没有额外的方法，实现的都是Collection原本定义的方法。向Set添加的对象，其所在的类一定要实现hashCode()和equals()方法。（hashCode()会在计算其索引值的时候用，equals()会在比较对象是否相同的时候用）；
...
是hashSet的子类，具有 HashSet 的查找效率，并且内部使用双向链表维护元素的插入顺序，对于频繁的遍历操作，效率比hashSet高。

##### TreeSet：

- 基于红黑树实现，支持有序性操作，例如根据一个范围查找元素的操作。但是查找效率不如 HashSet，HashSet 查找的时间复杂度为 O(1)，TreeSet 则为 O(logN)。
  - 可以按照添加对象的指定属性进行排序，所以要求该对象所在类必须可排序：自然排序（实现Comparable接口）和 定制排序（Comparator ,将comparator的对象当作参数传给TreeSet 构造器 ）。
- 自然排序中，比较两个对象是否相等就看compareTo()返回值是否是0.
...
private static final Object[] EMPTY_ELEMENTDATA = {};//容量为0的数组，创建时主动给capacity赋值
private static final Object[] DEFAULTCAPACITY_EMPTY_ELEMENTDATA = {};//默认容量的空数组,创建时先不给开辟内存，直到添加第一个元素时才开辟空间
```
...
- Hashtable 是线程安全的，使用 synchronized 来进行同步。所以 Hashtable 性能较差，HashMap 性能较好，使用更广。
- HashMap 可以插入键为 null 的 Entry。.Hashtable中，key和value都不允许出现null值。
...
#### 4.ConcurrentHashMap与 Hashtable 的比较
