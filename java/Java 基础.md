...
针对的问题：很多种类都有几乎一样的操作，但是如果针对每一种类都写一种操作类则过于麻烦，所以可以定义一种模板，在使用时具体说明实际的类。如Java标准库提供的ArrayList，它的内部维护了一个Object[]数组，配合存储一个当前分配的数组，就可以充当“可变数组”：
...
但是如果我们的使用情景要求，这个list里面只能存储String对象，那么我们就需要专门写一个StringArrayList，并在其add()方法中判断对象是否为String类的实例(直接由编译器针对类型检查)，以此类推，若是我们对Integer、Long、Person、Student等各种各样的类型都有这样的操作需求，如果为每一种类都单独编写一种XXXArrayList类则显得过于麻烦且会有很多冗余代码。
所以，我们可以把ArrayList变成一种模板ArrayList<T>，代码如下:
...
//创建可以存储Student的ArrayList:
ArrayList<Student> stuList = new ArrayList<Student>();
```

【由编译器做类型检查】，如此，既实现了编写一次，万能匹配，又通过编译器保证了类型安全，这就是范型。
...
public class Test2 {
   public static void main(String[] args) {
      ...
      Class classslist=slist.getClass();
      Class classilist=ilist.getClass();
      if(classslist.equals(classilist)) {
        System.out.println("泛型测试,类型相同");//"泛型测试,类型相同"
      }
   }
}
```
