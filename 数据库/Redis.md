...
场景：要求用户填写个人求职信息（学历、行业、技能、公司地点、薪资）之后从招聘库里查询并返回匹配的信息

问题：搜索很慢，加上搜索到相关结果之后后台还要做一定的筛选和匹配度计算，所以导致响应速度很慢。（我们的查找逻辑是全部扫描）

解决：数据库加了索引也无济于事。所以考虑用了redis，服务器启动时将所有的招聘信息、企业信息都<id，序列化（信息）>存在了resis里，之后查询的话，好像是查出所有的搜索结果的同时根据关键词过滤，一层层过滤（大佬写了个过滤器）。
